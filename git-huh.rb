...
HELP = <<EOS
git-huh displays the state of your repository in a readable, easy-to-scan
format. It's useful for getting a summary of how a branch relates to a remote
server, and for wrangling many topic branches.

git-huh can show you:
- How a branch relates to the remote repo, if it's a tracking branch.
- How a branch relates to integration branches, if it's a feature branch.
- How a branch relates to the feature branches, if it's an integration
  branch.

git-huh is best used before a git push, or between a git fetch and a git
...
that's a less common case and git-huh currently doesn't display anything
special for it.)
EOS

USAGE = <<EOS
Usage: git huh [branch+] [options]

If [branch] is not specified, git-huh will use the current branch. The possible
[options] are:
...
git-huh uses some heuristics to determine which branches are integration
branches, and which are feature branches. (Specifically, it assumes the
integration branches are named "master", "next" and "edge".) If it guesses
incorrectly, you will have to create a .git-huhrc file.

To start building a configuration file, run "git-huh --dump-config >
.git-huhrc" and edit it. The config file is a YAML file that specifies the
integration branches, any branches to ignore, and the max number of commits to
display when --all-commits isn't used.  git-huh will look for a .git-huhrc file
starting in the current directory, and recursively up to the root.

IMPORTANT NOTE: all local branches referenced in .git-huhrc must be prefixed
with heads/, e.g. "heads/master". Remote branches must be of the form
remotes/<remote>/<branch>.
EOS

COPYRIGHT = <<EOS
git-huh Copyright 2008--2009 William Morgan <wmorgan at the masanjin dot nets>.
...
want_color = `git config color.huh`
